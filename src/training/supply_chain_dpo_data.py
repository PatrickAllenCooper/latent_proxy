...
class SupplyChainDPOPairGenerator(DPOPairGenerator):
    """DPO pairs for the supply chain procurement domain."""

    def generate_dataset(
        self,
        supply_config: SupplyChainConfig | None = None,
    ) -> list[DPOPair]:
        cfg = supply_config or SupplyChainConfig(n_periods=30)
        ...
        alloc_serializer = SupplyChainAllocationSerializer(env)
        ...
        for state_idx in range(self.config.n_game_states):
            ...
            for _ in range(states_per_round):
                if self.config.curriculum_phase == 1:
                    pair = self._make_phase1(obs, env, candidate_gen, alloc_serializer)
                else:
                    user_type = sampler.sample()
                    pair = self._make_phase2(
                        obs, env, candidate_gen, alloc_serializer, user_type, sampler,
                    )
                if pair is not None:
                    pairs.append(pair)
        ...
        logger.info(
            "Generated %d supply chain DPO pairs for phase %d",
    ...
    def _make_phase1(
        ...
        alloc_serializer: SupplyChainAllocationSerializer,
    ) -> DPOPair | None:
        prompt = build_supply_chain_prompt(obs, env, user_type=None)
        candidates = candidate_gen.generate(self.config.n_candidates)

        scored: list[tuple[NDArray[np.floating[Any]], float, bool]] = []
        for c in candidates:
            q = env.quality_score(c)
            passes, _ = env.check_quality_floor(c)
            scored.append((c, q, passes))
    ...
    def _make_phase2(
        ...
        prompt = build_supply_chain_prompt(obs, env, user_type=user_type)
        target_theta = {
            "gamma": user_type.gamma, "alpha": user_type.alpha,
        ...
        for c in candidates:
            u = user.evaluate_allocation(c, means, variances, wealth, rounds_left)
            scored.append((c, u))
        scored.sort(key=lambda x: x[1], reverse=True)
        ...
        while (
            ...
            guard += 1
        contrast_theta = {
            "gamma": contrasting.gamma, "alpha": contrasting.alpha,
